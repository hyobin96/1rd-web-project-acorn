...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<?> handlePlayListNotDeleted(PlaylistNotDeletedException e) {
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("message", e.getMessage()));
	}

	// 플레이리스트가 생성이 되지 않은 경우
	@ExceptionHandler(PlaylistNotInsertedException.class)
	public ResponseEntity<?> handlePlayListNotInserted(PlaylistNotInsertedException e) {
		return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("message", e.getMessage()));
	}

	// 플레이리스트가 불려오지 않은 경우
	@ExceptionHandler(PlaylistNotSelectedException.class)
	public ResponseEntity<?> handlePlayListNotSelected(PlaylistNotSelectedException e) {
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("message", e.getMessage()));
	}
